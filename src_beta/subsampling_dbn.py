...
logger = logging.getLogger('subsampling_discrete_dbn_hypoexp_m1')


def construct_dbn_beta_hypoexp_m1_data(input_filename, output_filename, sampling_rate):
    """
    Function to construct discrete-time sub-sampled DBN data from Beta-Hypoexp/M/1 simulation data
    ----------
    Arguments:
    1. input_filename: CSV produced by the simulator with columns
       Run,Alpha,Beta,Phase_Rates,Mu,Current_Phase,IQL,End,Time,Event,Queue_Length
    ...
    for r in tqdm(range(1, runs + 1)):
        ...
        beta_run = d['Beta'].values[0]
        mu_run = d['Mu'].values[0]
        k_run = len(phase_rates)

        #time_series = []
        time_series = [alpha_run, beta_run]
    ...
    logger.info(f"Time taken to construct beta Hypoexp/M/1 DBN data: {end - start} seconds")
...
if __name__ == "__main__":
    ...
    time_series_experiment = config['beta_hypoexp_m1_time_series_experiment']
    input_file = f"simulation/beta_hypoexp-m1-{time_series_experiment}.csv"
    ...
    discrete_time_file = output_path / f"beta_hypoexp-discrete-time-dbn-exp-{experiment_number}.csv"

    # call the subsampling function
    construct_dbn_beta_hypoexp_m1_data(input_path, discrete_time_file, sampling_interval)
