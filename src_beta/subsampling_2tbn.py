...
def beta_hypoexp_construct_2tbn_data(input_filename, output_filename, sampling_rate):
    """
    Function to construct discrete-time sub-sampled data from Beta-Hypoexp/M/1 simulation data
    ----------
    Arguments:
    ...
    for r in tqdm(range(1, runs + 1)):  # Construct sampling data for each run
        ...
        # Keep Alpha/Beta in both slices for consistency with other variables
        # Rename d2 columns to represent current slice variables
        d2.columns = [
            'Alpha', 'Beta', 'Lambda', 'Mu', 'K', 'CurrentPhase', 'QueueLength'
        ]
...
if __name__ == "__main__":
    ...
    simulation_file = project_root / "data" / "simulation" / f"beta_hypoexp-m1-{time_series_experiment}.csv"
    output_path = project_root / "data" / output_folder
    output_path.mkdir(parents=True, exist_ok=True)
    discrete_time_file = output_path / f"beta_hypoexp-discrete-time-2tbn-exp-{experiment_number}.csv"
    ...
    beta_hypoexp_construct_2tbn_data(simulation_file, discrete_time_file, sampling_interval)
