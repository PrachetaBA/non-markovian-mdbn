...
def construct_dbn(bn_file,
    ...
    # Alpha and Beta are present in the 2TBN (one value per run). Collect their unique values.
    unique_alpha = sorted(data_bn['Alpha'].unique())
    unique_beta = sorted(data_bn['Beta'].unique())
    ...
    # Alpha/Beta (previous slice) — static per run but included as slice-0 variables
    alpha_tprev = gm.NumericalDiscreteVariable("Alpha0", "Alpha (t - 1)", unique_alpha)
    beta_tprev = gm.NumericalDiscreteVariable("Beta0", "Beta (t - 1)", unique_beta)
    ...
    # Alpha/Beta (current slice). They are constant but present for the current-slice parent set.
    alpha_t = gm.NumericalDiscreteVariable("Alphat", "Alpha (t)", unique_alpha)
    beta_t = gm.NumericalDiscreteVariable("Betat", "Beta (t)", unique_beta)

    # Create DBN and add variables
    dbn = gm.BayesNet()
    (alpha0, beta0, mu0, phase0, ql0) = [
        dbn.add(x) for x in [alpha_tprev, beta_tprev, mu_tprev, phase_tprev, ql_tprev]
    ]

    (alphat, betat, mut, phaset, qlt) = [
        dbn.add(x) for x in [alpha_t, beta_t, mu_t, phase_t, ql_t]
    ]
    ...
    # i_t  <- i_{t-1}, Alpha_t, Beta_t
    dbn.addArc(phase0, phaset)
    dbn.addArc(alphat, phaset)
    dbn.addArc(betat, phaset)

    # Print the DBN
    logger.debug(f"DBN: {dbn}")

    # Step 1. Ensure Alphat/Betat exist (they are identical to Alpha/Beta because static per run),
    ...
    data_bn["Beta0"] = pd.Categorical(data_bn["Beta0"], categories=unique_beta)
    data_bn["Alphat"] = pd.Categorical(data_bn["Alphat"], categories=unique_alpha)
    data_bn["Betat"] = pd.Categorical(data_bn["Betat"], categories=unique_beta)
    ...
    initial_vars = ["QueueLength0", "CurrentPhase0", "Alpha0", "Beta0", "Mu0"]
    ...
    simulation_vars = ["CurrentPhaset", "Alphat", "Betat", "Mut"]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config_file",
                        "-c",
                        type=str,
                        help="Path to the configuration file (e.g. config/beta_hypoexp_construct_dbn.yaml)",
                        default="config/beta_hypoexp_construct_dbn.yaml")
    ...
    parser.add_argument('--sim_config',
                        ...
                        default='config/hypoexp_beta_simulator.yaml',
                        required=False)
    ...
    expt_name = config.get('expt_name', 'beta_hypoexpm1_experiment')
    ...
    with open(args.sim_config, 'r', encoding='utf-8') as sim_file:
        ...
        sim_params = sim_config[f"experiment_{time_discretization_params['beta_hypoexp_m1_time_series_experiment']}"]
    ...
    bn_filename = (
        ...
        f"/beta_hypoexp-discrete-time-2tbn-exp-{time_discretization_experiment}.csv")
    dbn_filename = (
        f"{time_discretization_params['time_discretization_folder']}"
        f"/beta_hypoexp-discrete-time-dbn-exp-{time_discretization_experiment}.csv")
    ...
    if use_crosstab:
        CONSTRUCTED_DBN_FILENAME = project_root / f"data/discrete_time/dbn_beta_hypoexpm1_exp{time_discretization_experiment}_crosstab.bif"
    else:
        CONSTRUCTED_DBN_FILENAME = project_root / f"data/discrete_time/dbn_beta_hypoexpm1_exp{time_discretization_experiment}.bif"
