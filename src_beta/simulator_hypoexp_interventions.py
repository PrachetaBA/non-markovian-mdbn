...
import numpy as np
import pandas as pd
from beta_to_hypoexp import beta_hypoexponential_approximation
...
class BetaM1Simulation:
    """Class to simulate a beta HypoExp/M/1 queue."""
    ...
    def __init__(self,
        ...
        self.input_params = InputParameters(
            ...
            beta = query['start_parameters']['Beta'],
        ...
        self.t_arrival = self.clock + self.gen_int_arr(
            self.input_params.alpha, self.input_params.beta)
    ...
    def parameter_intervention(self, evidence_var, evidence_val):
        ...
        if evidence_var in ['Alpha', 'Beta']:

            # --- NEW: detect no-change intervention ---
            if evidence_var == 'Alpha' and evidence_val == self.input_params.alpha:
                return

            if evidence_var == 'Beta' and evidence_val == self.input_params.beta:
                return
            # -----------------------------------------

            new_shape = evidence_val if evidence_var == 'Alpha' else self.input_params.alpha
            new_scale = evidence_val if evidence_var == 'Beta' else self.input_params.beta

            key = (round(new_shape,6), round(new_scale,6))

            if key not in PH_CACHE:
                PH_CACHE[key] = beta_hypoexponential_approximation(new_shape, new_scale)

            self.input_params.alpha = new_shape
            self.input_params.beta = new_scale
...
if __name__ == "__main__":
    ...
    logger.debug(f'Config file: {config_file}, Experiment number: {experiment_number}')
    ...
    beta_shape = query_details['start_parameters']['Alpha']
    beta_scale = query_details['start_parameters']['Beta']
    ...
    key0 = (round(beta_shape,6), round(beta_scale,6))
    if key0 not in PH_CACHE:
        PH_CACHE[key0] = beta_hypoexponential_approximation(beta_shape, beta_scale)
    phase_rates_start = PH_CACHE[key0]

    for i in tqdm(range(gt_replications)):
        q = BetaM1Simulation(simulation_end=simulation_end_time,
                            query=query_details,
    ...
    summary_file = f"{temp_folder}/sim_beta_hypoexp_gt_exp_{experiment_number}.csv"
