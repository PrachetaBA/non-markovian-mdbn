...
"""
Simulation model of an G/M/1 queue with a Beta distribution.
Input parameters: 
    Beta: 
        alpha: shape
        beta: scale
...
class BetaM1Simulation:
    """Class to simulate a Beta/M/1 queue."""
    ...
    def arrival(self):
        ...
        self.t_arrival = self.clock + self.gen_int_arr(self.input_params.alpha, self.input_params.beta)
    ...
    def gen_int_arr(self, alpha, beta):
        """Function to generate the interarrival times using a Beta distribution."""
        return self.rng.beta(a=alpha, b=beta, size=1)[0]
    ...
    def parameter_intervention(self, evidence_var, evidence_val):
        ...
        if evidence_var == 'Alpha' or evidence_var == 'Beta':
            ...
            self.input_params.beta = evidence_val if evidence_var == 'Beta' else self.input_params.beta
...
if __name__ == "__main__":
    ...
    beta = query_details['start_parameters']['Beta']
    ...
    summary_file = f"{temp_folder}/sim_beta_gt_exp_{experiment_number}.csv"
    ...
    TIMESERIES_FILEPATH = f"{gt_folder}/gt-exp-{experiment_number}-beta.csv"
