...
"""Script to construct the DBN using the simulator data. 
...
# Set logger
logger = logging.getLogger('construct_dbn_logger')
...
def tm_l(bn2t_data, l0_val, prev_state_col, curr_state_col):
    ...
    We do not do data pooling as every queue has different parameters. 
...
def construct_dbn(bn_file,
    ...
    # Step 5. Learn the probabilities for the queue length variables for all time slices != 0

    # We do not do any parameter tying - learn each CPD independently.
    if 'indicator_ql' in edges:
        data_bn_qa = data_bn[[
        ...
        # For each Queue A, B, and C learn the transition matrix

        # Queue A
        logger.debug('Processing the queue length variable Lqat')
        for lambda_curr_val in unique_lambda_qa:
            for mu_curr_val in unique_mu_qa:
                for lambda_prev_val in unique_lambda_qc:
                    for mu_prev_val in unique_mu_qc:
                        for rca_curr_val in unique_r_ca:
                            for lqci0_val in [0, 1]:
                                for state in range(0, max_ql + 1):
                                    # Filter the dbn data to only contain
                                    # specific values of the parameters
                                    bn2t_data = data_bn_qa[
                                        (data_bn_qa['Lambdaqat'] ==
                                         lambda_curr_val) &
                                        (data_bn_qa['Muqat'] == mu_curr_val) &
                                        (data_bn_qa['Lqci0'] == lqci0_val) &
                                        (data_bn_qa['Lambdaqct']
                                         == lambda_prev_val) &
                                        (data_bn_qa['Muqct'] == mu_prev_val) &
                                        (data_bn_qa['Rcat'] == rca_curr_val)]
                                    # Learn the transition matrix for the queue lengths
                                    bn2t_data = bn2t_data[['Lqa0', 'Lqat']]
                                    ...
                                    tm_lt_prob = dict(sorted(
                                        tm_lt_prob.items()))
                                    # Set the CPT values
                                    dbn.cpt('Lqat')[{
                                        'Lambdaqat': str(lambda_curr_val),
                                        'Muqat': str(mu_curr_val),
                                        'Lambdaqct': str(lambda_prev_val),
                                        'Muqct': str(mu_prev_val),
                                        'Rcat': str(rca_curr_val),
                                        'Lqci0': lqci0_val,
                                        'Lqa0': state
                                    }] = list(tm_lt_prob.values())
        logger.debug('Finished processing the queue length variable Lqat')

        # Queue B
        logger.debug('Processing the queue length variable Lqbt')
        for lambda_curr_val in unique_lambda_qb:
            for mu_curr_val in unique_mu_qb:
                for lambda_prev_val in unique_lambda_qa:
                    for mu_prev_val in unique_mu_qa:
                        for rab_curr_val in unique_r_ab:
                            for lqai0_val in [0, 1]:
                                for state in range(0, max_ql + 1):
                                    # Filter the dbn data to only contain
                                    # specific values of the parameters
                                    bn2t_data = data_bn_qb[
                                        (data_bn_qb['Lambdaqbt'] ==
                                         lambda_curr_val) &
                                        (data_bn_qb['Muqbt'] == mu_curr_val) &
                                        (data_bn_qb['Lqai0'] == lqai0_val) &
                                        (data_bn_qb['Lambdaqat']
                                         == lambda_prev_val) &
                                        (data_bn_qb['Muqat'] == mu_prev_val) &
                                        (data_bn_qb['Rabt'] == rab_curr_val)]
                                    bn2t_data = bn2t_data[['Lqb0', 'Lqbt']]
                                    tm_lt_prob = tm_l(bn2t_data,
                                                      state,
                                                      prev_state_col='Lqb0',
                                                      curr_state_col='Lqbt')
                                    ...
                                    # Set the CPT values
                                    dbn.cpt('Lqbt')[{
                                        'Lambdaqbt': str(lambda_curr_val),
                                        'Muqbt': str(mu_curr_val),
                                        'Lambdaqat': str(lambda_prev_val),
                                        'Muqat': str(mu_prev_val),
                                        'Rabt': str(rab_curr_val),
                                        'Lqai0': lqai0_val,
                                        'Lqb0': state
                                    }] = list(tm_lt_prob.values())
        logger.debug('Finished processing the queue length variable Lqbt')

        # Queue C
        logger.debug('Processing the queue length variable Lqct')
        for lambda_curr_val in unique_lambda_qc:
            for mu_curr_val in unique_mu_qc:
                for lambda_prev_val in unique_lambda_qb:
                    for mu_prev_val in unique_mu_qb:
                        for rbc_curr_val in unique_r_bc:
                            for lqbi0_val in [0, 1]:
                                for state in range(0, max_ql + 1):
                                    # Filter the dbn data to only contain
                                    # specific values of the parameters
                                    bn2t_data = data_bn_qc[
                                        (data_bn_qc['Lambdaqct'] ==
                                         lambda_curr_val) &
                                        (data_bn_qc['Muqct'] == mu_curr_val) &
                                        (data_bn_qc['Lambdaqbt']
                                         == lambda_prev_val) &
                                        (data_bn_qc['Muqbt'] == mu_prev_val) &
                                        (data_bn_qc['Rbct'] == rbc_curr_val) &
                                        (data_bn_qc['Lqbi0'] == lqbi0_val)]
                                    # Learn the transition matrix for the queue lengths
                                    bn2t_data = bn2t_data[['Lqc0', 'Lqct']]
                                    tm_lt_prob = tm_l(bn2t_data,
                                                      state,
                                                      prev_state_col='Lqc0',
                                                      curr_state_col='Lqct')
                                    ...
                                    # Set the CPT values
                                    dbn.cpt('Lqct')[{
                                        'Lambdaqct': str(lambda_curr_val),
                                        'Muqct': str(mu_curr_val),
                                        'Lambdaqbt': str(lambda_prev_val),
                                        'Muqbt': str(mu_prev_val),
                                        'Rbct': str(rbc_curr_val),
                                        'Lqbi0': lqbi0_val,
                                        'Lqc0': state
                                    }] = list(tm_lt_prob.values())
        logger.debug('Finished processing the queue length variable Lqct')
    else:
        data_bn_qa = data_bn[[
        ...
        for lambda_curr_val in unique_lambda_qa:
            for mu_curr_val in unique_mu_qa:
                for lambda_prev_val in unique_lambda_qc:
                    for mu_prev_val in unique_mu_qc:
                        for rca_curr_val in unique_r_ca:
                            for lqc0_val in range(0, max_ql + 1):
                                for state in range(0, max_ql + 1):
                                    # Filter the dbn data to only contain
                                    # specific values of the parameters
                                    bn2t_data = data_bn_qa[
        ...
        for lambda_curr_val in unique_lambda_qb:
            for mu_curr_val in unique_mu_qb:
                for lambda_prev_val in unique_lambda_qa:
                    for mu_prev_val in unique_mu_qa:
                        for rab_curr_val in unique_r_ab:
                            for lqa0_val in range(0, max_ql + 1):
                                for state in range(0, max_ql + 1):
                                    # Filter the dbn data to only contain
                                    # specific values of the parameters
                                    bn2t_data = data_bn_qb[
        ...
        for lambda_curr_val in unique_lambda_qc:
            for mu_curr_val in unique_mu_qc:
                for lambda_prev_val in unique_lambda_qb:
                    for mu_prev_val in unique_mu_qb:
                        for rbc_curr_val in unique_r_bc:
                            for lqb0_val in range(0, max_ql + 1):
                                for state in range(0, max_ql + 1):
                                    # Filter the dbn data to only contain
                                    # specific values of the parameters
                                    bn2t_data = data_bn_qc[
...
if __name__ == "__main__":
    ...
    Function call: 
    python src/construct_dbn.py --config_file configs/queries.yaml
