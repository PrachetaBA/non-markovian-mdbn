...
def construct_dbn(bn_file,
    ...
    lambda_tprev = gm.NumericalDiscreteVariable("Lambda0", "Arrival rate (t-delta)", unique_lambda)
    mu_tprev = gm.NumericalDiscreteVariable("Mu0", "Service rate (t-delta)", unique_mu)
    k_tprev = gm.NumericalDiscreteVariable("K0", "Erlang phases (t-delta)", unique_k)
    phase_tprev = gm.NumericalDiscreteVariable("CurrentPhase0", "Current service phase (t-delta)", unique_phases)
    ql_tprev = gm.RangeVariable("QueueLength0", "Queue length (t-delta)", 0, max_ql)

    # Current time-slice variables
    lambda_t = gm.NumericalDiscreteVariable("Lambdat", "Arrival rate (t)", unique_lambda)
    mu_t = gm.NumericalDiscreteVariable("Mudat", "Service rate (t)", unique_mu)
    k_t = gm.NumericalDiscreteVariable("Kdat", "Erlang phases (t)", unique_k)
    phase_t = gm.NumericalDiscreteVariable("CurrentPhaset", "Current service phase (t)", unique_phases)
    ...
    dbn.addArc(mudat, qlt)          # Mudat -> QueueLengtht
    ...
    data_bn.columns = [
        ...
        "Lambdat", "Mudat", "Kdat", "CurrentPhaset", "QueueLengtht"
    ]
    ...
    data_bn["Mudat"] = pd.Categorical(data_bn["Mudat"], categories=unique_mu)
    data_bn["Kdat"] = pd.Categorical(data_bn["Kdat"], categories=unique_k)
    ...
    for col, cats in [
        ...
        if col in data_dbn.columns:
            ...
            # Not fatal: if full time-series has different naming, user should ensure presence of slice-0 columns
            logger.debug(f"Column {col} not present in data_dbn; skip categorical conversion.")
    ...
    data_bn_ql = data_bn[[
        'QueueLength0', 'CurrentPhase0', 'Lambdat', 'Mudat', 'QueueLengtht'
    ]]
