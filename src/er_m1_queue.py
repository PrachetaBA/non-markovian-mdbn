...
class ErM1Simulation:
    ...
    def generate_Q_matrix(self, max_queue_length: int):
        """
        builds generator matrix Q for Er/M/1 queue
        state = (phase, queue_length)
        """
        num_phases = self.input_params.k
        arrival_phase_rate = num_phases / self.input_params.mean_interarrival_time
        ...
        for queue_len in range(max_queue_length + 1):
            for phase in range(1, num_phases + 1):
                ...
                if phase < num_phases:
                    ...
                    Q[state_idx, next_state_idx] = arrival_phase_rate
                else:
                ...
                # set diagnoal entries so rows add up to 1
