...
def single_intervention(config_file,
    ...
    unq_wb_shapes = tuple(sorted(sim_config['WB_SHAPES']))
    unq_wb_scales = tuple(sorted(sim_config['WB_SCALES']))
    ...
    parameter_variables = ['WBshape', 'WBscale', 'Mu']
    ...
    for query_num in range(start_exp_num, start_exp_num + total_queries):
        start_wb_shape = rng.choice(unq_wb_shapes)
        start_wb_scale = rng.choice(unq_wb_scales)
        ...
        if intervention_type in [
            ...
            if intervention_variable == 'WBshape':
                intervention_value = rng.choice([
                    val for val in unq_wb_shapes
                    if val != start_wb_shape
                ])
            elif intervention_variable == 'WBscale':
                intervention_value = rng.choice([
                    val for val in unq_wb_scales
                    if val != start_wb_scale
                ])
        ...
        exp_skeleton = {
            ...
            'maximum_ql':
                int(maximum_ql),
            'start_parameters': {
                'WBshape': start_wb_shape,
                'WBscale': start_wb_scale,
                'Mu': start_mu,
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Generate query workload for weibull experiments.')
    ...
    expt_dets = {
        2: {
           'time_discretization_experiment': 2,
           'simulation_config_expt': 2,
           'max_ql':22, 
           'inference_algorithm': 'LazyPropagation',
           'dbn_name': 'dbn_weibull_hypoexpm1_exp5'
        }, 
        5: {
            'time_discretization_experiment': 5,
            'simulation_config_expt': 5,
            'max_ql':22,
            'inference_algorithm': 'LazyPropagation',
            'dbn_name': 'dbn_weibull_hypoexpm1_exp5'
        }
    ...
    time_discretization_filename = 'config/weibull_hypoexp_time_discretization.yaml'
    ...
    CONFIG_NAME = (
        f'weibull_query_workload_exp-{EXPT_NUM}')
    ...
    single_intervention(
        ...
        simulation_config_file='config/weibull_simulator.yaml',
