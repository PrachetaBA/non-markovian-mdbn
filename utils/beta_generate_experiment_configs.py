...
def single_intervention(config_file,
    ...
    unq_alpha = tuple(sorted(sim_config['B_ALPHAS']))
    unq_beta = tuple(sorted(sim_config['B_BETAS']))
    ...
    parameter_variables = ['Alpha', 'Beta', 'Mu']
    ...
    for query_num in range(start_exp_num, start_exp_num + total_queries):
        ...
        start_beta = rng.choice(unq_beta)
        ...
        if intervention_type in [
            ...
            elif intervention_variable == 'Beta':
                intervention_value = rng.choice([
                    val for val in unq_beta
                    if val != start_beta
                ])
...
if __name__ == '__main__':
    ...
    expt_dets = {
        1: { # actuall exp
           ...
           'max_ql': 21, 
           'inference_algorithm': 'LazyPropagation',
           'dbn_name': 'dbn_beta_hypoexpm1_exp1'
        }, 
        2: { # just for baseline
        ...
        3: { # actual exp with different time discretization, to see if it affects the results
            'time_discretization_experiment': 3,
            'simulation_config_expt': 3,
            'max_ql': 21,
            'inference_algorithm': 'LazyPropagation',
    ...
    time_discretization_filename = 'config/beta_hypoexp_time_discretization.yaml'
    ...
    CONFIG_NAME = (
        f'beta_query_workload_exp-{EXPT_NUM}')
    ...
    single_intervention(
        ...
        simulation_config_file='config/beta_simulator.yaml',
