...
if __name__ == "__main__":
    ...
    outpath = output_root / f'weibull_hypoexp-m1-{args.experiment_number}.csv'
    ...
    for dist in arrival_distributions:
        ...
        wb_shape = dist["wb_shape"]
        wb_scale = dist["wb_scale"]
        ...
        for mu in service_rates:
            ...
            for iql in iql_values:
                ...
                for i in range(runs):
                    seed = base_seed + i
                    sim = HypoExpM1Simulation(
                        phase_rates=phase_rate_set,
                    ...
                    df_list.append(
                        sim.time_series.assign(
                            Run=RUN,
                            WB_SHAPE=wb_shape,
                            WB_SCALE=wb_scale,
                            Phase_Rates=str(phase_rate_set),
                            Mu=mu,
                            IQL=iql,
                            End=sim_end
                        )
            ...
            logger.info(f"Optimal subsampling interval for wb_shape={wb_shape}, wb_scale={wb_scale}, mu={mu} = {delta}")
    ...
    if len(df_list) > 0:
        ...
        df = df[["Run","WB_SHAPE","WB_SCALE","Phase_Rates","Mu","Current_Phase","IQL","End","Time","Event","Queue_Length"]]
