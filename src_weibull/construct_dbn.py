...
def construct_dbn(bn_file,
    ...
    # WBshape and WBscale are present in the 2TBN (one value per run). Collect their unique values.
    unique_wb_shape = sorted(data_bn['WBshape'].unique())
    unique_wb_scale = sorted(data_bn['WBscale'].unique())
    ...
    # WBshape/WBscale (previous slice) — static per run but included as slice-0 variables
    wb_shape_tprev = gm.NumericalDiscreteVariable("WBshape0", "WBshape (t - 1)", unique_wb_shape)
    wb_scale_tprev = gm.NumericalDiscreteVariable("WBscale0", "WBscale (t - 1)", unique_wb_scale)
    ...
    # WBshape/WBscale (current slice). They are constant but present for the current-slice parent set.
    wb_shape_t = gm.NumericalDiscreteVariable("WBshapet", "WBshape (t)", unique_wb_shape)
    wb_scale_t = gm.NumericalDiscreteVariable("WBscalet", "WBscale (t)", unique_wb_scale)

    # Create DBN and add variables
    dbn = gm.BayesNet()
    (wb_shape0, wb_scale0, mu0, phase0, ql0) = [
        dbn.add(x) for x in [wb_shape_tprev, wb_scale_tprev, mu_tprev, phase_tprev, ql_tprev]
    ]

    (wb_shape_t, wb_scale_t, mut, phaset, qlt) = [
        dbn.add(x) for x in [wb_shape_t, wb_scale_t, mu_t, phase_t, ql_t]
    ]
    ...
    # i_t  <- i_{t-1}, wb_shape_t, wb_scale_t
    dbn.addArc(phase0, phaset)
    dbn.addArc(wb_shape_t, phaset)
    dbn.addArc(wb_scale_t, phaset)

    # Print the DBN
    logger.debug(f"DBN: {dbn}")

 
    # Step 1. Ensure WBshape/WBscale exist (they are identical to WBshape/WBscale because static per run),
    ...
    data_bn["WBshape0"] = pd.Categorical(data_bn["WBshape0"], categories=unique_wb_shape)
    data_bn["WBscale0"] = pd.Categorical(data_bn["WBscale0"], categories=unique_wb_scale)
    data_bn["WBshapet"] = pd.Categorical(data_bn["WBshapet"], categories=unique_wb_shape)
    data_bn["WBscalet"] = pd.Categorical(data_bn["WBscalet"], categories=unique_wb_scale)
    ...
    for col, cats in [
        ("WBshape0", unique_wb_shape),
        ("WBscale0", unique_wb_scale),
        ("Mu0", unique_mu),
    ...
    initial_vars = ["QueueLength0", "CurrentPhase0", "WBshape0", "WBscale0", "Mu0"]
    ...
    simulation_vars = ["CurrentPhaset", "WBshapet", "WBscalet", "Mut"]
    ...
    for mu_value in unique_mu:
        for phase_value in unique_phases:
            ...
            if use_crosstab:
                ...
                fname = f"weibull_experiment{exp_no}_crosstab_mu{mu_value}_phase{phase_value}.png"
            else:
                plt.title(f"TML: μ={mu_value}, phase={phase_value}")
                fname = f"weibull_experiment{exp_no}_tml_mu{mu_value}_phase{phase_value}.png"
...
if __name__ == "__main__":
    """Construct and save the Weibull Hypoexp/M/1 DBN for a given config.

    Usage example:
      python construct_dbn.py --config_file configs/queries.yaml --experiment_number 1 -v
    """
    parser = argparse.ArgumentParser(description="Construct the DBN for the Weibull Hypoexp M1 queueing system.")
    parser.add_argument("--config_file",
                        "-c",
                        type=str,
                        help="Path to the configuration file (e.g. configs/queries.json)",
                        default="config/weibull_hypoexp_construct_dbn.yaml")
    ...
    expt_name = config.get('expt_name', 'weibull_hypoexpm1_experiment')
    ...
    with open(args.sim_config, 'r', encoding='utf-8') as sim_file:
        ...
        sim_params = sim_config[f"experiment_{time_discretization_params['weibull_hypoexp_m1_time_series_experiment']}"]
    ...
    bn_filename = (
        ...
        f"/weibull_hypoexp-discrete-time-2tbn-exp-{time_discretization_experiment}.csv")
    dbn_filename = (
        f"{time_discretization_params['time_discretization_folder']}"
        f"/weibull_hypoexp-discrete-time-dbn-exp-{time_discretization_experiment}.csv")
    ...
    if use_crosstab:
        CONSTRUCTED_DBN_FILENAME = project_root / f"data/discrete_time/dbn_weibull_hypoexpm1_exp{time_discretization_experiment}_crosstab.bif"
    else:
        CONSTRUCTED_DBN_FILENAME = project_root / f"data/discrete_time/dbn_weibull_hypoexpm1_exp{time_discretization_experiment}.bif"
