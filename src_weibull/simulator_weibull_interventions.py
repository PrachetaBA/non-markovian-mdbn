...
"""
Simulation model of an G/M/1 queue with a Weibull distribution.
Input parameters: 
    Weibull: 
        wb_shape: shape
        wb_scale: scale
...
class WeibullM1Simulation:
    """Class to simulate a Weibull/M/1 queue."""
    ...
    def arrival(self):
        ...
        self.t_arrival = self.clock + self.gen_int_arr(self.input_params.wb_shape, self.input_params.wb_scale)
    ...
    def gen_int_arr(self, wb_shape, wb_scale):
        """Function to generate the interarrival times using a Weibull distribution."""
        #return self.rng.weibull(a=wb_shape, scale=wb_scale, size=1)[0] # TODO: Check correct rate vs. inverse
        return wb_scale * self.rng.weibull(a=wb_shape, size=1)[0]
    ...
    def parameter_intervention(self, evidence_var, evidence_val):
        ...
        if evidence_var == 'WBshape' or evidence_var == 'WBscale':
            #setattr(self.input_params, evidence_var, evidence_val)
            self.input_params.wb_shape = evidence_val if evidence_var == 'WBshape' else self.input_params.wb_shape
            self.input_params.wb_scale = evidence_val if evidence_var == 'WBscale' else self.input_params.wb_scale
...
if __name__ == "__main__":
    ...
    for i in tqdm(range(gt_replications)):
        q = WeibullM1Simulation(simulation_end=simulation_end_time,
                            query=query_details,
                            seed=i + 123)
    ...
    summary_file = f"{temp_folder}/sim_weibull_gt_exp_{experiment_number}.csv"
    ...
    TIMESERIES_FILEPATH = f"{gt_folder}/gt-exp-{experiment_number}-weibull.csv"
