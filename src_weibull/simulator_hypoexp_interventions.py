...
import numpy as np
import pandas as pd
from weibull_to_hypoexp import weibull_hypoexponential_approximation
...
class GammaM1Simulation:
    ...
    def __init__(self,
        ...
        #phase_rates = weibull_hypoexponential_approximation(
        #    query['start_parameters']['WBshape'],
        #    query['start_parameters']['WBscale'])
        phase_rates = phase_rates_start

        self.input_params = InputParameters(
            wb_shape = query['start_parameters']['WBshape'],
            wb_scale = query['start_parameters']['WBscale'],
        ...
        self.t_arrival = self.clock + self.gen_int_arr(
            self.input_params.wb_shape, self.input_params.wb_scale)
        ...
        row = pd.Series([
            self.input_params.wb_shape,
            self.input_params.wb_scale,
    ...
    def parameter_intervention(self, evidence_var, evidence_val):
        ...
        if evidence_var in ['WBshape', 'WBscale']:

            # --- NEW: detect no-change intervention ---
            if evidence_var == 'WBshape' and evidence_val == self.input_params.wb_shape:
                return

            if evidence_var == 'WBscale' and evidence_val == self.input_params.wb_scale:
                return
            # -----------------------------------------

            new_shape = evidence_val if evidence_var == 'WBshape' else self.input_params.wb_shape
            new_scale = evidence_val if evidence_var == 'WBscale' else self.input_params.wb_scale

            key = (round(new_shape,6), round(new_scale,6))

            if key not in PH_CACHE:
                PH_CACHE[key] = weibull_hypoexponential_approximation(new_shape, new_scale)

            self.input_params.wb_shape = new_shape
            self.input_params.wb_scale = new_scale
            self.input_params.phase_rates = PH_CACHE[key]
...
if __name__ == "__main__":
    ...
    wb_shape = query_details['start_parameters']['WBshape']
    wb_scale = query_details['start_parameters']['WBscale']
    ...
    key0 = (round(wb_shape,6), round(wb_scale,6))
    if key0 not in PH_CACHE:
        PH_CACHE[key0] = weibull_hypoexponential_approximation(wb_shape, wb_scale)
    phase_rates_start = PH_CACHE[key0]

    for i in tqdm(range(gt_replications)):
        q = GammaM1Simulation(simulation_end=simulation_end_time,
                            query=query_details,
                            phase_rates_start = phase_rates_start,
                            seed=i + 123)
    ...
    summary_file = f"{temp_folder}/sim_weibull_hypoexp_gt_exp_{experiment_number}.csv"
