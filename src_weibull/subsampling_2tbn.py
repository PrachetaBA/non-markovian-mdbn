...
def weibull_hypoexp_construct_2tbn_data(input_filename, output_filename, sampling_rate):
    """
    Function to construct discrete-time sub-sampled data from Weibull/Hypoexp/M/1 simulation data
    ----------
    Arguments:
    ...
    global_df = pl.DataFrame(schema=[
        ("WBshape_tprev", pl.Float64),
        ("WBscale_tprev", pl.Float64),
    ...
    for r in tqdm(range(1, runs + 1)):  # Construct sampling data for each run
        ...
        wb_shape_run = float(d[0, "WB_SHAPE"])
        wb_scale_run = float(d[0, "WB_SCALE"])
        ...
        while t <= simulation_end:
            ...
            time_series = pl.DataFrame({
                "WBshape_tprev": [wb_shape_run],
                "WBscale_tprev": [wb_scale_run],
        ...
        last_row = pl.DataFrame({
            'WBshape_tprev': [last_row_values[0]],
            'WBscale_tprev': [last_row_values[1]],
        ...
        # Keep WBshape/WBscale in both slices for consistency with other variables
        # Rename d2 columns to represent current slice variables
        d2.columns = [
            'WBshape', 'WBscale', 'Lambda', 'Mu', 'K', 'CurrentPhase', 'QueueLength'
        ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        help=
        "Path to the configuration file (e.g. configs/weibull_hypoexp_time_discretization.yaml)",
    ...
    simulation_file = project_root / "data" / "simulation" / f"weibull_hypoexp-m1-{time_series_experiment}.csv"
    output_path = project_root / "data" / output_folder
    output_path.mkdir(parents=True, exist_ok=True)
    discrete_time_file = output_path / f"weibull_hypoexp-discrete-time-2tbn-exp-{experiment_number}.csv"
    ...
    weibull_hypoexp_construct_2tbn_data(simulation_file, discrete_time_file, sampling_interval)
